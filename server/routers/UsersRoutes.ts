...
import { validUser, UserModel, validLogin, createToken } from "../models/userModel";
...
router.post("/login", async (req, res) => {
    ...
    try {
        ...
        if (!user) {
            return res.status(401).json({ msg: "User or password are wrong" });
        }
        ...
        // Generate token
        const newToken = createToken(user._id.toString(), user.role); // Convert _id to string
